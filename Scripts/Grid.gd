...
func placePart(mouse_x,mouse_y,part: RobotPart) -> bool:
	for tile in _grid:
		tile.setClear()	
	...
	for x in range(0,partArray2d.getWidth()):
		for y in range(0,partArray2d.getHeight()):
			...
			if tile_value and partArray2d.getItem(x,y):
				return false
	for x in range(0,partArray2d.getWidth()):
		for y in range(0,partArray2d.getHeight()):	
			var part_value = partArray2d.getItem(x,y)
			if part_value:
				_grid.getItem(x+arrayPos.x,y+arrayPos.y).setValue(part.value)
...
func hoverPart(mouse_x,mouse_y,part: RobotPart) -> bool:
	...
	for x in range(0,partArray2d.getWidth()):
		for y in range(0,partArray2d.getHeight()):
			...
			if (tile.getValue()) and partArray2d.getItem(x,y):
				tile.setForbidden()
